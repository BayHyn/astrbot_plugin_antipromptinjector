...
* **核心防御模式 (`defense_mode`)**:
  * 强烈建议您通过 `/切换防护模式` 指令来管理此项。当然，您也可以在设置界面进行选择。默认为 `sentry` (哨兵模式)。
...
由于作者持续的那么一个懒，平常不会及时的看issues，所以开了个QQ反馈群方便用户以及时的鞭打作者。拷打作者

群号 ： 928985352   进群密码 ： 神人desuwa
